...
class_identifier
  ...
  | TYPE_IDENTIFIER
      { perm_string name = lex_strings.make($1.text);
	class_type_t*tmp = new class_type_t(name);
	FILE_NAME(tmp, @1);
	pform_set_typedef(@1, name, tmp, NULL);
	delete[]$1.text;
...
type_declaration
  ...
  | K_typedef typedef_basic_type IDENTIFIER ';'
      { perm_string name = lex_strings.make($3);
  ...
  | K_typedef TYPE_IDENTIFIER ';'
      { perm_string name = lex_strings.make($2.text);
	pform_forward_typedef(@2, name, typedef_t::ANY);
	delete[]$2.text;
      }
  | K_typedef typedef_basic_type TYPE_IDENTIFIER ';'
      { perm_string name = lex_strings.make($3.text);
	pform_forward_typedef(@3, name, $2);
	delete[]$3.text;
      }
  | K_typedef K_enum TYPE_IDENTIFIER ';'
      { perm_string name = lex_strings.make($3.text);
	pform_forward_typedef(@3, name, typedef_t::ENUM);
	delete[]$3.text;
      }
	...
	| list_of_port_declarations ',' IDENTIFIER
		{ Module::port_t*ptmp;
		  ...
		  pform_module_define_port(@3, name,
					...
					port_declaration_context.data_type, 0, 0);
		  delete[]$3;
		  $$ = tmp;
		}
	| list_of_port_declarations ','
...
port_declaration
  : attribute_list_opt K_input net_type_opt data_type_or_implicit IDENTIFIER dimensions_opt
	...
	pform_module_define_port(@2, name, NetNet::PINPUT, $3, $4, $6, 0, $1);
  ...
  | attribute_list_opt K_input net_type_opt data_type_or_implicit IDENTIFIER '=' expression
	...
	pform_module_define_port(@2, name, NetNet::PINPUT, $3, use_type, 0, $1);
	port_declaration_context.port_type = NetNet::PINPUT;
	port_declaration_context.port_net_type = $3;
	port_declaration_context.data_type = $4;
	delete[]$5;
  ...
  | attribute_list_opt K_inout net_type_opt data_type_or_implicit IDENTIFIER dimensions_opt
	...
	perm_string name = lex_strings.make($5);
	ptmp = pform_module_port_reference(@2, name);
	pform_module_define_port(@2, name, NetNet::PINOUT, $3, $4, 0, $1);
  ...
  | attribute_list_opt K_output net_type_opt data_type_or_implicit IDENTIFIER '=' expression
	...
	ptmp = pform_module_port_reference(@2, name);
	pform_module_define_port(@2, name, NetNet::POUTPUT, use_type, $4, 0, $1);
	port_declaration_context.port_type = NetNet::PINOUT;
	port_declaration_context.port_net_type = use_type;
	port_declaration_context.data_type = $4;
...
parameter_assign
  : IDENTIFIER parameter_value_ranges_opt
      { pform_set_parameter(@1, lex_strings.make($1), param_is_local,
			    param_is_type, param_data_type, 0, $2);
	delete[]$1;
      }
  | IDENTIFIER '=' expression parameter_value_ranges_opt
      { PExpr*tmp = $3;
	pform_set_parameter(@1, lex_strings.make($1), param_is_local,
			    param_is_type, param_data_type, tmp, $4);
	delete[]$1;
      }
  ;
