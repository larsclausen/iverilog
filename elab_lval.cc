...
NetAssign_*PEIdent::elaborate_lval_var_(Design *des, NetScope *scope,
					...
					NetNet *reg, pform_name_t tail_path) const
{
	...
	// net_class_member_ method.
      if (reg->class_type() && !tail_path.empty() && gn_system_verilog()) {
	    NetAssign_*lv = elaborate_lval_net_class_member_(des, scope, reg, tail_path);
	    return lv;
      }
